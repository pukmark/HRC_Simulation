...
class CollaborativeGame():
    def __init__(self, N: int = 10, dt: float = 0.1, d: float = 4.0, delta_d: float = 0.01, Obstcles: list = None, verbose: int = 0):
        ...
        J2 = 0.5*(((1.0-avoid_Obs)*ca.sumsqr(x2[:,0]-x2_f[0]) + (1.0-avoid_Obs)*ca.sumsqr(x2[:,1]-x2_f[1])) + 0.5*0.1*ca.sumsqr(v2)  + 0.1*ca.sumsqr(a2)) + 1e5*ca.sumsqr(Slack) + 1e5*ca.sumsqr(Slack_OA)
        ...
        scOA_vec = []
        for k in range(N):
            scL_vec.append(self.d_max**2 + Slack[k]**2 - ca.sumsqr(x1[k+1,:]-x2[k+1,:])) # d_max**2 - (x1-x2)**2 >=0
            scL_vec.append(ca.sumsqr(x1[k+1,:]-x2[k+1,:]) - self.d_min**2 - Slack[k]**2) # (x1-x2)**2 - d_min**2 >=0
        ...
        sigL1 = ca.SX.sym('scL1', scL_vec.shape[0])
        sigL2 = ca.SX.sym('scL2', scL_vec.shape[0])
        sigOA1 = ca.SX.sym('scOA1', scOA_vec.shape[0])
        sigOA2 = ca.SX.sym('scOA2', scOA_vec.shape[0])

        L1 -= ca.dot(sigL1, scL_vec)
        L2 -= ca.dot(sigL2, scL_vec)
        L1 -= ca.dot(sigOA1, scOA_vec)
        L2 -= ca.dot(sigOA2, scOA_vec)
        
        sc_vec = ca.vertcat(scL_vec, scL_vec, scOA_vec, scOA_vec)
        sig = ca.vertcat(sigL1, sigL2, sigOA1, sigOA2)

        self.Z = ca.vertcat(x1[:], v1[:], a1[:], x2[:], v2[:], a2[:], Slack[:], Slack_OA[:], mu1[:], mu2[:], lam1[:], lam2[:], sig[:])
        ...
        _Dxu_L.append( ca.jacobian(L2, ca.vertcat(x2[:], v2[:], a2[:], Slack[:], Slack_OA[:])).T )
        ...
        self.fun_F = ca.Function('F', [self.Z, x1_0, v1_0, x1_f, x2_0, v2_0, x2_f, avoid_Obs], [F])
        J = ca.jacobian(F, self.Z)
        self.fun_J = ca.Function('J', [self.Z, x1_0, v1_0, x1_f, x2_0, v2_0, x2_f, avoid_Obs], [J])
    ...
    def Solve(self, time, x1_0, v1_0, x1_f, x2_0, v2_0, x2_f, z0 = None, avoid_Obs = 0.0, log: bool = True):
        ...
        Main.F_py = lambda z: np.array(self.fun_F(z, x1_0, v1_0, x1_f, x2_0, v2_0, x2_f, avoid_Obs)).squeeze()
        Main.J_py = lambda z: np.array(self.fun_J(z, x1_0, v1_0, x1_f, x2_0, v2_0, x2_f, avoid_Obs))
        ...
        _f = np.array(self.fun_F(z, x1_0, v1_0, x1_f, x2_0, v2_0, x2_f, avoid_Obs)).squeeze()
